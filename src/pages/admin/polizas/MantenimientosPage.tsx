...
export function MantenimientosPage() {
  ...
  const { data: polizas = [], isLoading: loadingPolizas } = usePolizasQuery()
  const { mutate: actualizarMantenimiento, isPending: isUpdating } = useEditarMantenimientoMutation()
  const isPageLoading = isLoading || loadingPolizas
  ...
  }, [data, deferredSearch, fechaDesde, fechaHasta, selectedPolizaId, status])
  ...
  const { totalPendientes, totalEnRuta, totalRealizados } = useMemo(() => {
    let pendientes = 0
    let enRuta = 0
    let realizados = 0

    for (const mantenimiento of filteredMantenimientos) {
      if (mantenimiento.status === 'pendiente') pendientes += 1
      if (mantenimiento.status === 'en_ruta') enRuta += 1
      if (mantenimiento.status === 'realizado') realizados += 1
    }
  ...
  return (
    <div className="p-5 lg:p-7">
      ...
      {isPageLoading ? (
        <>
          <AdminFilterBarSkeleton
            className="mb-4 lg:grid-cols-[1.5fr_0.95fr_0.75fr_0.75fr_1.25fr]"
            items={['', '', '', '', '']}
          />
          <AdminStatsGridSkeleton
            count={3}
