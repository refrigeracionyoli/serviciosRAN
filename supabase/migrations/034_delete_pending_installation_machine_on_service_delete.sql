-- When an installation service is deleted before completion, delete the
-- temporary machine that was created only for that service.
...
declare
  ...
  v_pending_installation_machine_id bigint;
begin
  ...
  if public.normalize_workshop_service_type(v_servicio.tipo_servicio) like '%INSTALACION%'
     and v_servicio.maquina_id is not null then
    select maquinas.id
    into v_pending_installation_machine_id
    from public.maquinas
    where maquinas.id = v_servicio.maquina_id
      and maquinas.activo = true
      and maquinas.status = 'en_taller'
      and maquinas.cliente_id is null
      and maquinas.fecha_instalacion is null
      and not exists (
        select 1
        from public.servicios other_servicios
        where other_servicios.maquina_id = maquinas.id
          and other_servicios.id <> p_servicio_id
      )
      and not exists (
        select 1
        from public.polizas
        where polizas.maquina_id = maquinas.id
      )
      and not exists (
        select 1
        from public.mantenimientos_poliza
        where mantenimientos_poliza.maquina_id = maquinas.id
      )
      and not exists (
        select 1
        from public.maquinas_en_taller
        where maquinas_en_taller.maquina_id = maquinas.id
      )
      and not exists (
        select 1
        from public.maquinas_taller_movimientos
        where maquinas_taller_movimientos.maquina_id = maquinas.id
      )
    limit 1;
  end if;
  ...
  if v_pending_installation_machine_id is not null then
    delete from public.maquinas
    where maquinas.id = v_pending_installation_machine_id;
  end if;
